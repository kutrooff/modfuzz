...
def check_status_code(result: ExecutionResult) -> None:
    ...
    expected_values = [int(val) for val in expected]

    if not expected:
        result.checks.append("status_code: skipped")
        return

    if result.status_code in expected_values:
        result.checks.append("status_code: passed")
    else:
        result.success = False
        result.checks.append(
            f"status_code: failed, expected {expected_values}, got {result.status_code}"
        )
